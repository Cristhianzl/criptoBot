...
SOCKET = "wss://stream.binance.com:9443/ws/aaveusdt@kline_1m"
...
def buy_or_sell(type):
    ...
    if(type=='sell'): #sell order
        if in_position == True:
            ...
            order_succeeded = order(SIDE_SELL, TRADE_SYMBOL)
            if order_succeeded:
                in_position = False
        else:
...
def main():
    ...
    #montar requisição na api pra pegar o preço
    ...
    variation = float((lastPrice - openPrice) / (openPrice))*100
